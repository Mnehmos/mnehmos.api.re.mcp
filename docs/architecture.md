...
| `apire/capture/` | Transport listeners. One module per transport under a common `Listener` protocol: `start/stop/status` yield normalized `RawFrame`s into the store. The *only* module allowed to touch sockets, with the connect policy of ADR-003. |
| `apire/normalize/` | `http.py`, `ws.py`, `sse.py`, `pipe.py`, `logs.py`, `jsbundle.py`, `process.py` — each produces the common IR (`Observation`) with a canonical shape key. |
